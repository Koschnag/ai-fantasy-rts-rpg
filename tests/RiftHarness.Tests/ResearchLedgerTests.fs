...
module private T053 =
    ...
    let workspace action =
        let root =
            Path.Combine(Path.GetTempPath(), "RiftHarness.T053-" + Guid.NewGuid().ToString("N"))
    ...
    let payload toolClass =
        json
            $"{{\"toolClass\":\"{toolClass}\",\"commandDigest\":\"{sha 'b'}\",\"startedMonotonicNs\":1,\"completedMonotonicNs\":2,\"resultSha256\":\"{sha 'c'}\"}}"
    ...
    let append root ledger event =
        ResearchLedger.append root ledger event |> ignore
...
module ResearchLedgerTests =
    ...
    let canonicalRoundTrip () =
        workspace (fun root ->
            ...
            assertEqual
                (ResearchLedger.canonicalEventBytes event |> ResearchCanonical.appendLf)
                persisted
    ...
    let tamperIsNotRecoverableTail () =
        workspace (fun root ->
            ...
            let tampered =
                ...
                + "\"toolClass\":\"tampered\""
                + original.Substring(index + needle.Length)

            File.WriteAllText(ledger, tampered, Constants.Utf8NoBom)
            let result = ResearchLedger.verify root ledger

            assertEqual
                ResearchLedgerStatus.Invalid
                result.Status
                "Changed non-final event was treated as recoverable tail.")
    ...
    let unknownIsLiteralAndNullIsRejected () =
        workspace (fun root ->
            ...
            assertTrue
                (persisted.Contains("\"providerId\":\"unknown\"", StringComparison.Ordinal))
                "Unknown provider was not explicit."
            ...
            let invalid =
                draft
                    ...
                    (json
                        "{\"toolClass\":null,\"commandDigest\":\"unknown\",\"startedMonotonicNs\":\"unknown\",\"completedMonotonicNs\":\"unknown\",\"resultSha256\":\"unknown\"}")
    ...
    let tornTailRequiresExplicitRecovery () =
        workspace (fun root ->
            ...
            use stream =
                new FileStream(ledger, FileMode.Append, FileAccess.Write, FileShare.None)
            ...
            let recoveryPayload =
                json
                    $"{{\"originalLedgerSha256\":\"{originalHash}\",\"verifiedPrefixSha256\":\"{torn.VerifiedPrefixSha256}\",\"tornTailSha256\":\"{Option.get torn.TornTailSha256}\",\"recoveredLedgerPath\":\"{recoveredRelative}\"}}"
    ...
    let exclusiveWriterRejectsConcurrency () =
        workspace (fun root ->
            ...
            use held =
                new FileStream(
                    ResearchLedger.lockPath ledger,
    ...
    let monotonicClockAllowsWallClockSkewOnly () =
        workspace (fun root ->
            ...
            let first =
                { synthetic 1 observation with
                    OccurredAtUtc = ResearchValue.Known "2026-09-02T09:59:59.000Z"
                    MonotonicClockId = ResearchValue.Known "clock-a"
                    MonotonicTimeNs = ResearchValue.Known 10L }

            let skewed =
                { synthetic 2 observation with
                    RecordedAtUtc = "2026-09-02T10:00:01.000Z"
                    OccurredAtUtc = ResearchValue.Known "2026-09-02T09:59:58.000Z"
                    MonotonicClockId = ResearchValue.Known "clock-a"
                    MonotonicTimeNs = ResearchValue.Known 11L }
            ...
            assertEqual
                ...
                (ResearchLedger.verify root ledger).Status
                "Wall-clock skew invalidated monotonic evidence."

            let reversed =
                { synthetic 3 observation with
                    RecordedAtUtc = "2026-09-02T10:00:02.000Z"
                    MonotonicClockId = ResearchValue.Known "clock-a"
                    MonotonicTimeNs = ResearchValue.Known 9L }
        ...
        workspace (fun root ->
            ...
            for forbidden in
                [ "hunter2"
                  ...
                  "provider-private-42" ] do
                assertTrue
                    (not (persisted.Contains(forbidden, StringComparison.Ordinal)))
                    $"Secret persisted: {forbidden}"

            assertTrue (persisted.Contains("[REDACTED:", StringComparison.Ordinal)) "Typed redaction marker is absent."

            expectFailure "RESEARCH_SCHEMA_INVALID" (fun () ->
                append
                    root
                    ledger
                    (draft 1 observation "synthetic-test-only" "2026-09-02T10:00:00.000Z" secretPayload))
    ...
    let evidenceClassesStaySeparated () =
        workspace (fun root ->
            ...
            let prospective =
                { draft 2 observation "prospective-observed" "2026-09-02T10:00:01.000Z" (payload "test") with
                    SourceRefs = [ source "agent-event" ] }
            ...
            assertEqual
                ...
                (ResearchLedger.readVerified root prospectiveLedger |> List.head).Body.EvidenceClass
                "Evidence class changed in roundtrip.")
    ...
    let harnessEventReferencesAreStrictlyEarlierAndHashBound () =
        workspace (fun root ->
            ...
            let candidate number referenceValue =
                { synthetic number observation with
                    SourceRefs = [ referenceValue ] }
            ...
            |> List.iteri (fun index (_, sourceValue) ->
                ...
                let draft =
                    { candidate (index + 2) sourceValue with
                        ObservationId = target }

                let targetLedger =
                    if index = 4 then
                        ResearchLedger.ledgerPath root target
                    else
                        ledger
            ...
            let valid =
                candidate 20 (reference (ResearchValue.Known earlier.Body.EventId) earlier.EventHash)

            append root ledger valid

            assertEqual
                2
                (ResearchLedger.readVerified root ledger).Length
                "Valid earlier harness-event reference was rejected.")
    ...
    let collectorPayloadContractsRejectMalformedValues () =
        workspace (fun root ->
            ...
            let validPayload =
                $"{{\"agentId\":\"agent-test\",\"agentRole\":\"builder\",\"promptSha256\":\"{sha 'a'}\",\"toolchainSha256\":\"{sha 'b'}\"}}"
            ...
            let cases =
                [ "missing",
                  validPayload.Replace(",\"toolchainSha256\":\"" + sha 'b' + "\"", "", StringComparison.Ordinal)
                  "null",
            ...
            assertEqual
                1
                (ResearchLedger.readVerified root ledger).Length
                "Valid strict collector payload was rejected.")
    ...
    let gateAndVerificationPayloadContractsAreTypedAndExact () =
        workspace (fun root ->
            ...
            let gatePayload =
                $"{{\"attempt\":1,\"gateId\":\"gate-1\",\"targetTreeId\":\"{tree}\"}}"

            let verificationPayload =
                $"{{\"attempt\":1,\"evidenceSha256\":\"{evidence}\",\"stageId\":\"gate-1\",\"targetTreeId\":\"{tree}\"}}"
            ...
            let gateCases =
                [ "RESEARCH_SCHEMA_INVALID",
                  gatePayload.Replace("\"attempt\":1", "\"attempt\":\"1\"", StringComparison.Ordinal)
                  "RESEARCH_SCHEMA_INVALID",
                  gatePayload.Replace("\"attempt\":1", "\"attempt\":0", StringComparison.Ordinal)
                  "RESEARCH_SCHEMA_INVALID", gatePayload.Replace(tree, "not-an-object-id", StringComparison.Ordinal)
                  "RESEARCH_JSON_INVALID",
                  gatePayload.Replace(
                      "\"gateId\":\"gate-1\"",
                      "\"gateId\":\"gate-1\",\"gateId\":\"gate-2\"",
                      StringComparison.Ordinal
                  )
                  "RESEARCH_SCHEMA_INVALID", gatePayload.Substring(0, gatePayload.Length - 1) + ",\"extra\":true}" ]

            gateCases
            |> List.iteri (fun index (failureCode, payload) ->
                expectFailure failureCode (fun () -> append root ledger (draft "gate.started" (index + 1) payload)))
            // The preregistered contract requires a positive integer attempt;
            // only the target tree may remain explicitly unknown.
            let unknownPayload =
                "{\"attempt\":1,\"gateId\":\"gate-unknown\",\"targetTreeId\":\"unknown\"}"
            ...
            let failureCases =
                [ "missing",
                  verificationPayload.Replace(",\"evidenceSha256\":\"" + evidence + "\"", "", StringComparison.Ordinal)
                  "string-attempt",
                  verificationPayload.Replace("\"attempt\":1", "\"attempt\":\"1\"", StringComparison.Ordinal)
                  "bad-evidence", verificationPayload.Replace(evidence, "not-a-hash", StringComparison.Ordinal)
                  "extra",
                  verificationPayload.Substring(0, verificationPayload.Length - 1)
                  + ",\"extra\":true}" ]

            failureCases
            |> List.iteri (fun index (_, payload) ->
                expectFailure "RESEARCH_SCHEMA_INVALID" (fun () ->
                    append root ledger (draft "verify.failed" (index + 11) payload)))
            ...
            assertEqual
                3
                (ResearchLedger.readVerified root ledger).Length
                "Typed gate and verification payloads were rejected.")
    ...
    let closedLifecycleAndPrivacyAreFailClosed () =
        workspace (fun root ->
            ...
            appendEvent
                4
                "outcome.observed"
                $"{{\"taskOutcome\":\"accepted\",\"hypothesisResult\":\"inconclusive\",\"resultCommit\":\"{String('e', 40)}\",\"resultTreeId\":\"{String('f', 40)}\",\"reasonCode\":\"test\"}}"
            ...
            let privatePaths =
                { synthetic 7 (observationId 16) with
                    ChangedFiles = ResearchValue.Known 1L
                    ChangedPaths = ResearchValue.Known [ "/Users/alice/secret.txt" ] }
