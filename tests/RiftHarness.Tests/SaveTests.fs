...
let saveContractMirrorsDocumentedValues () =
    ...
    if
        SaveContract.ContractVersion <> "3"
        || SaveContract.CurrentSaveSchemaVersion <> 2us
        || SaveContract.LegacySaveSchemaVersion <> 1us
    then
        failwith "Vertragsversion falsch (V3 mit Legacy V1 und unveraenderter Umschlagsversion 2)."
    ...
    for identifier in
        [ SaveContract.EncodingId
          ...
          SaveContract.FinalityFixtureDeferralStatement
          SaveContract.SessionSectionModelId
          SaveContract.HeadlessActivationModelId
          SaveContract.InteractiveActivationModelId
          SaveContract.CodecBoundaryModelId
          SaveContract.LegacyEmptinessModelId
          SaveContract.ActivationGuardModelId
          SaveContract.SessionPersistenceReplayExceptionStatement
          SaveContract.LegacyV1SessionEmptinessStatement
          SaveContract.SaveSlotActionName
          SaveContract.LoadSlotActionName ] do
    ...
    // V2-Struktur: das Dokument dokumentiert die additive Sektion, die
    // unterscheidbaren Sektionsklassen und die Replay-Ausnahme.
    for anchor in
        [ "sessionSectionVersion"
          "SESSION_SECTION_INTEGRITY_VIOLATION"
          "SESSION_SECTION_INVALID"
          "saveSchemaVersion"
          "MaxSessionSectionBytes"
          "legacy-v1-session-emptiness-v2"
          "untrusted-slot-activation-guards-v2"
          "foreign-world-id"
          "foreign-seed"
          "layer-activation-mismatch" ] do
