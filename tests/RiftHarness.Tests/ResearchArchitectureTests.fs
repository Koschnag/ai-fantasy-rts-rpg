...
module ResearchArchitectureTests =
    ...
    let private baseInput files =
        { CheckpointId = "checkpoint-1"
          ...
          PathMap =
            [ { Prefix = "src/Riftward.App"; FileClass = "production"; Component = "Riftward.App" }
              { Prefix = "src/Riftward.Session"; FileClass = "production"; Component = "Riftward.Session" }
              { Prefix = "tests"; FileClass = "test"; Component = "tests" } ]
          Files = files
          BaselineReferences = []
          ResultReferences = []
          Findings = []
    ...
    let binaryIsUnknown () =
        let snapshot =
            baseInput [ { Path = "src/Riftward.App/Asset.bin"; BaselinePath = None; ResultLines = Some 4; BaselineLines = Some 1; IsBinary = true; SourceSha256 = "binary"; Changed = true } ]
            |> ResearchArchitecture.create
        require (snapshot.FileRows.Head.Lines = "unknown") "binary line count must remain unknown"
        require (snapshot.TrendRows |> List.exists (fun row -> row.Metric = "binary-files-changed" && row.Value = "1")) "binary metric missing"

    let renamedFilePreservesBaselineName () =
        let snapshot =
            baseInput [ { Path = "src/Riftward.App/NewName.cs"; BaselinePath = Some "src/Riftward.App/OldName.cs"; ResultLines = Some 13; BaselineLines = Some 8; IsBinary = false; SourceSha256 = "source"; Changed = true } ]
            |> ResearchArchitecture.create
    ...
    let emptyPathMapAblatesClassification () =
        let input = baseInput [ { Path = "src/Riftward.App/File.cs"; BaselinePath = None; ResultLines = Some 2; BaselineLines = Some 1; IsBinary = false; SourceSha256 = "source"; Changed = true } ]
        ...
        require (snapshot.TrendRows |> List.exists (fun row -> row.Metric = "production-lines" && row.Value = "unknown")) "ablated production lines must be unknown"

    let deterministicBytes () =
        let files =
            [ { Path = "tests/A.fs"; BaselinePath = None; ResultLines = Some 2; BaselineLines = Some 1; IsBinary = false; SourceSha256 = "a"; Changed = true }
              { Path = "src/Riftward.App/A.cs"; BaselinePath = None; ResultLines = Some 3; BaselineLines = Some 2; IsBinary = false; SourceSha256 = "b"; Changed = true } ]
    ...
    let specialIntegrationPointsAreExplicit () =
        let files =
            ...
            |> List.map (fun name -> { Path = $"src/Riftward.App/Command/{name}.cs"; BaselinePath = None; ResultLines = Some 3; BaselineLines = Some 1; IsBinary = false; SourceSha256 = name; Changed = true })
            |> fun rows -> { Path = "src/Riftward.Session/SessionEngine.cs"; BaselinePath = None; ResultLines = Some 4; BaselineLines = Some 2; IsBinary = false; SourceSha256 = "session"; Changed = true } :: rows
        let snapshot = baseInput files |> ResearchArchitecture.create
        require (snapshot.IntegrationRows |> List.map (fun row -> row.Name) = [ "CommandLoopRunner"; "CommandReportSchema"; "SessionEngine" ]) "special integration set changed"
    ...
    let neverGateCoupled () =
        ...
        require (snapshot.TrendRows |> List.forall (fun row -> not row.GateCoupled)) "trend inventory became gate coupled"
        require (not (eventDocument.RootElement.GetProperty("gateCoupled").GetBoolean())) "event payload became gate coupled"

    let hashBoundArtifactsRoundTrip () =
        let snapshot =
            baseInput [ { Path = "src/Riftward.App/A.cs"; BaselinePath = None; ResultLines = Some 3; BaselineLines = Some 2; IsBinary = false; SourceSha256 = "source"; Changed = true } ]
            |> ResearchArchitecture.create
