...
- Die opt-in Abschluss- und Wiederholungsschicht (T-039) ist eine rein sitzungsseitige Beobachtungs- und Semantikschicht in `Riftward.Session` über demselben Kern und der Kette T-034 bis T-036 (versionierter Vertrag `docs/ABSCHLUSSVERTRAG.md` V1): An jeder Vorgrenze wird die feste Ordnung Intents, Erkundung, Entscheidung, Druck, Abschluss eingehalten; der abgeleitete Abschlusszustand ist eine reine Funktion der bestehenden Schichtwahrheiten der aktuellen Kette (Druckendstatus `success` plus abgeschlossene Entscheidung plus abgeschlossene Erkundung) ohne neue persistenzpflichtige Abschlussbytes. Die wirksame Wiederholen-Aktion (Skriptgrammatik `graybox-input-script-v4` plus Keymap-Aktion `repeat-mission`) setzt ausschließlich im Abschlusszustand an ihrer Vorgrenze die gesamte sitzungslokale Kette kontrolliert zurück — Aufsuchprotokoll samt Fortschritt, Entscheidungsangebot/Wahl/Folge, Druckfenster/Zyklus; die Kettenlaufzählung erhöht sich um genau eins; der Sitzungsmodus, die Welt und die Simulation bleiben unverändert, und es gibt keinen Welt- oder Prozessneustart aus der Schicht (ADR 008; Out-of-Session-Semantik ausgeschlossen). Die Schicht erzeugt niemals einen Kernbefehl und gehört zu keinem Zeitpunkt zu Simulationszustand oder Hashkette; die Kettenlaufzählung ist als additive Sektionsversion 2 des Savevertrags V3 in Save/Load fortsetzbar (ehrliche Legacy-Sektionsleere, Replay-Ausnahme besteht), die abgeleitete Abschlusswahrheit selbst trägt kein Persistenzbyte; die Aktivierung ist vertraglich an `--pressure` gekoppelt (`--mission` ohne `--pressure` ist Usage-Fehlanwendung) und hebt den Report rein additiv auf Schemaversion 7 mit `gateCoupled=false`. Der Titel-HUD-Abschlussausweis ist ein rein darstellseitiger Kanal (NF-005); `Riftward.Simulation` kennt keinen Abschluss, keine Kette und keine Wiederholung.
