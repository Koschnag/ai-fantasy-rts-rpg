...
- `kommandoschleife --scenario kommando-graybox --input-script PFAD --seed N --report PFAD [--interactive] [--capture-frame PFAD] [--warmup-ticks N] [--horizon-ticks N] [--lock DATEI]`: erste interaktive Graybox-Kommandoschleife (T-032) nativ auf linux-x64 im bestehenden Host ausführen — headless rein CPU-seitig ohne Fenster/Renderer/Netzwerk (native Artefakte werden nicht geladen) oder fensterpflichtig mit `--interactive` über denselben Pipelinepfad. Der Sitzungskern bildet validierte Intents ausschließlich auf die unveränderte Kernbefehlsfläche (`SimCommandKind.GroupMoveToZone`) ab und folgt dem Kommandovertrag `docs/KOMMANDOVERTRAG.md` V1; der Report (Schemaversion 1) bindet Skript-/Planhashes, Zustands-Hashketten, Tickzeit-, Allokations- und Reaktionsticksverteilung mit Einheit und Methode, und das Gate entscheidet fail-closed ausschließlich gegen die dokumentierten absoluten Grenzwerte (16 ms hart/8 ms Ziel je Tick, 0 Bytes Allokation je warmem Tick, Reaktion ≤ 3 Ticks hart/≤ 2 Ziel, keine Laufzeitshaderkompilierung, headless Kettenkonsistenz). Unbekanntes Szenario oder malformiertes Skript schlägt mit Exitcode 37 ohne Report fehl; Gateverletzungen ergeben 35, ein unvollständiger Lauf 36 ohne Evidenz, ein fehlgeschlagener opt-in Abgriff 38 mit `captured=false`; dieselben Profil-Ehrlichkeitsregeln wie bench-empty
