...
module ResearchLedger =
    let private eventIdPattern = Regex("^EV-[A-Z0-9]{26}$", RegexOptions.CultureInvariant ||| RegexOptions.NonBacktracking)
    let private observationIdPattern = Regex("^OBS-[A-Z0-9]{26}$", RegexOptions.CultureInvariant ||| RegexOptions.NonBacktracking)
    let private taskIdPattern = Regex("^T-[0-9]{3,}$", RegexOptions.CultureInvariant ||| RegexOptions.NonBacktracking)

    let private timestampPattern =
        Regex("^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}\\.[0-9]{3}Z$", RegexOptions.CultureInvariant ||| RegexOptions.NonBacktracking)

    let private hashOrUnknown value = value = ResearchContract.Unknown || Internal.isSha256 value
    ...
    let private eventFields =
        set
            [ "schemaVersion"; "eventId"; "studyId"; "observationId"; "runId"; "parentRunId"; "cycleId"; "taskId"
              "sequence"; "previousEventHash"; "monotonicTimeNs"; "monotonicClockId"; "occurredAtUtc"; "recordedAtUtc"
              "evidenceClass"; "eventType"; "actorRole"; "actorId"; "providerId"; "modelId"; "modelVersion"; "branchRef"
              "baseCommit"; "headCommit"; "treeId"; "autonomyMode"; "activityState"; "result"; "exitCode"; "failureClass"
              "retryIndex"; "repairIndex"; "usageProvenance"; "costProvenance"; "requestCount"; "inputTokens"; "outputTokens"
              "cacheReadTokens"; "cacheWriteTokens"; "costAmount"; "costCurrency"; "changedFiles"; "changedPaths"; "linesAdded"
              "linesDeleted"; "binaryFilesChanged"; "privacyClass"; "redactionStatus"; "redactionPolicyVersion"
              "humanActiveDurationMs"; "sourceRefs"; "payload"; "supersedesEventId"; "eventHash" ]

    let private sourceFields =
        set [ "sourceKind"; "repositoryCommit"; "repositoryPath"; "lineStart"; "lineEnd"; "artifactSha256"; "sourceEventId"; "resolvable" ]
    ...
    let private computeHash (event: ResearchEvent) = eventBytes false event |> Internal.sha256Hex
    ...
    let private parseIntValue (name: string) (element: JsonElement) : ResearchValue<int64> =
        ...
        if value.ValueKind = JsonValueKind.String && value.GetString() = ResearchContract.Unknown then
    ...
    let private parseSourceRef (element: JsonElement) =
        ...
        if resolvable.ValueKind <> JsonValueKind.True && resolvable.ValueKind <> JsonValueKind.False then
    ...
    let private parseEvent (line: byte array) =
        try
            ...
            if sequenceElement.ValueKind <> JsonValueKind.Number || not (sequenceElement.TryGetInt64(&sequence)) then
            ...
            if schemaVersionElement.ValueKind <> JsonValueKind.Number || not (schemaVersionElement.TryGetInt32(&schemaVersion)) then
    ...
    let private validateTimestamp (name: string) (value: string) =
        ...
        if not (DateTimeOffset.TryParseExact(value, "yyyy-MM-dd'T'HH:mm:ss.fff'Z'", CultureInfo.InvariantCulture, DateTimeStyles.AssumeUniversal ||| DateTimeStyles.AdjustToUniversal, &parsed)) then
    ...
    let private validateEnum (name: string) (allowed: Set<string>) (value: ResearchValue<string>) =
        match value with
        | ResearchValue.Known known when not (Set.contains known allowed) -> fail "RESEARCH_SCHEMA_INVALID" $"Unsupported {name} '{known}'."
        | _ -> ()

    let private validateCommit (name: string) (value: ResearchValue<string>) =
        match value with
        | ResearchValue.Known known when not (Regex.IsMatch(known, "^[0-9a-f]{40}$|^[0-9a-f]{64}$", RegexOptions.CultureInvariant)) ->
    ...
    let private validateRelativePath (name: string) (value: string) =
        if String.IsNullOrWhiteSpace(value) || Path.IsPathRooted(value) || value.Contains('\\') then
            fail "RESEARCH_SCHEMA_INVALID" $"{name} must be a non-empty repo-relative slash path."

        let segments = value.Split('/')

        if segments |> Array.exists (fun segment -> segment = "" || segment = "." || segment = "..") then
            fail "RESEARCH_SCHEMA_INVALID" $"{name} contains an unsafe segment."

    let private validateEventBody (body: ResearchEventBody) =
        if body.SchemaVersion <> ResearchContract.SchemaVersion then fail "RESEARCH_SCHEMA_INVALID" "Unsupported schemaVersion."
        if body.StudyId <> ResearchContract.StudyId then fail "RESEARCH_SCHEMA_INVALID" "Unsupported studyId."
        if not (eventIdPattern.IsMatch(body.EventId)) then fail "RESEARCH_SCHEMA_INVALID" "eventId has invalid syntax."
        if not (observationIdPattern.IsMatch(body.ObservationId)) then fail "RESEARCH_SCHEMA_INVALID" "observationId has invalid syntax."
        if not (Set.contains body.EvidenceClass ResearchContract.EvidenceClasses) then fail "EVIDENCE_CLASS_INVALID" "Unsupported evidenceClass."
        if not (Set.contains body.EventType ResearchContract.EventTypes) then fail "EVENT_TYPE_INVALID" $"Unsupported eventType '{body.EventType}'."
        ...
        | ResearchValue.Known occurred ->
            ...
            if recorded < parsed then fail "RESEARCH_SCHEMA_INVALID" "recordedAtUtc precedes occurredAtUtc."
        | ResearchValue.Unknown -> ()
        ...
        | ResearchValue.Known taskId when not (taskIdPattern.IsMatch(taskId)) -> fail "RESEARCH_SCHEMA_INVALID" "taskId has invalid syntax."
        | _ -> ()
        ...
        [ "exitCode", body.ExitCode; "retryIndex", body.RetryIndex; "repairIndex", body.RepairIndex; "requestCount", body.RequestCount
          "inputTokens", body.InputTokens; "outputTokens", body.OutputTokens; "cacheReadTokens", body.CacheReadTokens
          "cacheWriteTokens", body.CacheWriteTokens; "changedFiles", body.ChangedFiles; "linesAdded", body.LinesAdded
          "linesDeleted", body.LinesDeleted; "binaryFilesChanged", body.BinaryFilesChanged; "humanActiveDurationMs", body.HumanActiveDurationMs ]
        |> List.iter (fun (name, value) -> validateNonNegative name value)

        let usageValues =
            [ body.RequestCount; body.InputTokens; body.OutputTokens; body.CacheReadTokens; body.CacheWriteTokens ]

        if usageValues |> List.exists (function ResearchValue.Known _ -> true | ResearchValue.Unknown -> false) then
            match body.UsageProvenance with
            | ResearchValue.Unknown -> fail "RESEARCH_SCHEMA_INVALID" "Known request/token usage requires usageProvenance."
            | ResearchValue.Known _ -> ()

        [ "baseCommit", body.BaseCommit; "headCommit", body.HeadCommit; "treeId", body.TreeId ]
        ...
        | ResearchValue.Known paths ->
            ...
            if paths <> (paths |> List.distinct |> List.sortWith (fun left right -> StringComparer.Ordinal.Compare(left, right))) then
                fail "RESEARCH_SCHEMA_INVALID" "changedPaths must be ordinal-sorted and deduplicated."

            match body.ChangedFiles with
            | ResearchValue.Known count when count <> int64 paths.Length -> fail "RESEARCH_SCHEMA_INVALID" "changedFiles must equal changedPaths length."
            | _ -> ()
        | ResearchValue.Unknown -> ()
        ...
        | ResearchValue.Known amount, ResearchValue.Known currency, ResearchValue.Known _ ->
            let mutable cost = 0M
            if not (Regex.IsMatch(amount, "^(0|[1-9][0-9]*)(\\.[0-9]+)?$", RegexOptions.CultureInvariant))
            ...
            if not (Regex.IsMatch(currency, "^[A-Z]{3}$", RegexOptions.CultureInvariant)) then fail "RESEARCH_SCHEMA_INVALID" "costCurrency must be ISO-style uppercase code."
        | _ -> fail "RESEARCH_SCHEMA_INVALID" "costAmount, costCurrency, and costProvenance must all be known or all unknown."
        ...
        if List.isEmpty body.SourceRefs then fail "RESEARCH_SCHEMA_INVALID" "sourceRefs cannot be empty."

        for source in body.SourceRefs do
            if not (Set.contains source.SourceKind ResearchContract.SourceKinds) then fail "RESEARCH_SCHEMA_INVALID" $"Unsupported sourceKind '{source.SourceKind}'."
            if not (Internal.isSha256 source.ArtifactSha256) then fail "RESEARCH_SCHEMA_INVALID" "sourceRefs[].artifactSha256 must be lowercase SHA-256."
            validateCommit "sourceRefs[].repositoryCommit" source.RepositoryCommit
            match source.RepositoryPath with ResearchValue.Known path -> validateRelativePath "sourceRefs[].repositoryPath" path | _ -> ()
            ...
            | ResearchValue.Known first, _ when first < 1L -> fail "RESEARCH_SCHEMA_INVALID" "sourceRefs[].lineStart is 1-based."
            | _, ResearchValue.Known last when last < 1L -> fail "RESEARCH_SCHEMA_INVALID" "sourceRefs[].lineEnd is 1-based."
            | ResearchValue.Known first, ResearchValue.Known last when first > last -> fail "RESEARCH_SCHEMA_INVALID" "sourceRefs line range is reversed."
            | _ -> ()
            ...
            if source.Resolvable then
                ...
                if not hasAddress && not (source.SourceKind = "harness-event" && source.RepositoryPath = ResearchValue.Unknown) then
                    fail "RESEARCH_SCHEMA_INVALID" "A resolvable sourceRef requires a repositoryPath or sourceEventId address."

        if body.EvidenceClass <> "synthetic-test-only" && body.SourceRefs |> List.forall (fun source -> source.SourceKind = "fixture") then
            fail "EVIDENCE_CLASS_INVALID" "Non-synthetic evidence cannot rely only on fixture sourceRefs."

        if body.Payload.ValueKind <> JsonValueKind.Object then fail "RESEARCH_SCHEMA_INVALID" "payload must be an object."
        ...
        for field in required do
            if not (body.Payload.TryGetProperty(field) |> fst) then fail "RESEARCH_SCHEMA_INVALID" $"payload.{field} is required for {body.EventType}."
        ...
        let payloadString field =
            ...
            if value.ValueKind <> JsonValueKind.String then fail "RESEARCH_SCHEMA_INVALID" $"payload.{field} must be a string."
            value.GetString()

        let payloadHash field =
            let value = payloadString field
            if value <> ResearchContract.Unknown && not (Internal.isSha256 value) then fail "RESEARCH_SCHEMA_INVALID" $"payload.{field} must be SHA-256 or unknown."

        let payloadObjectId field =
            let value = payloadString field
            if value <> ResearchContract.Unknown
               && not (Regex.IsMatch(value, "^[0-9a-f]{40}$|^[0-9a-f]{64}$", RegexOptions.CultureInvariant)) then
        ...
        | "agent.run.started" ->
            ...
            if agentRole <> "builder" && agentRole <> "reviewer" && agentRole <> ResearchContract.Unknown then fail "RESEARCH_SCHEMA_INVALID" "payload.agentRole is invalid."
        ...
        | "gate.started" ->
            validateAttemptAndTargetPayload [ "attempt"; "gateId"; "targetTreeId" ] "gateId"
        | "gate.finished" ->
        ...
        | "activity.state.changed" ->
            ...
            for field in [ "fromActivityState"; "toActivityState" ] do
                let value = payloadString field
                if value <> ResearchContract.Unknown && not (Set.contains value ResearchContract.ActivityStates) then fail "RESEARCH_SCHEMA_INVALID" $"payload.{field} has an invalid activity state."
        | _ -> ()

        if body.EventType = "wip.snapshot.created" then
            let value = requireProperty "continuityOnly" body.Payload
            if value.ValueKind <> JsonValueKind.True then fail "RESEARCH_SCHEMA_INVALID" "wip.snapshot.created requires continuityOnly=true."

        if body.EventType = "architecture.checkpoint.created" then
            let value = requireProperty "gateCoupled" body.Payload
            if value.ValueKind <> JsonValueKind.False then fail "RESEARCH_SCHEMA_INVALID" "architecture checkpoints require gateCoupled=false."

        if body.EventType = "research.intervention.recorded" && requireString "durationMs" body.Payload <> ResearchContract.Unknown then
        ...
        if body.EventType = "agent.run.started" then
            ...
            | ResearchValue.Known actor when actor <> payloadAgent -> fail "RESEARCH_SCHEMA_INVALID" "agent.run.started payload.agentId must equal actorId."
            | ResearchValue.Unknown when payloadAgent <> ResearchContract.Unknown -> fail "RESEARCH_SCHEMA_INVALID" "Known payload.agentId requires the same known actorId."
            | _ -> ()

        let interventionCategoryField =
            if body.EventType.StartsWith("research.intervention.", StringComparison.Ordinal) then Some "category"
            elif body.EventType.StartsWith("human.", StringComparison.Ordinal) then Some "interventionCategory"
            else None
        ...
        | Some field when body.EventType <> "research.intervention.ended" ->
            ...
            if category <> ResearchContract.Unknown && not (Set.contains category ResearchContract.InterventionCategories) then
                fail "RESEARCH_SCHEMA_INVALID" $"Unsupported intervention category '{category}'."

            let counted = requireProperty "counted" body.Payload
            if counted.ValueKind <> JsonValueKind.True && counted.ValueKind <> JsonValueKind.False then
            ...
            if category <> ResearchContract.Unknown && category <> "I0-observation-no-intervention" && not (counted.GetBoolean()) then
    ...
    let private validateEventBoundSources (body: ResearchEventBody) (earlier: ResearchEvent list) =
        for source in body.SourceRefs do
            ...
            if source.SourceKind = "harness-event" && source.RepositoryPath = ResearchValue.Unknown then
                match source.SourceEventId with
                | ResearchValue.Unknown -> fail "RESEARCH_SOURCE_INVALID" "harness-event without repositoryPath requires sourceEventId."
                ...
                | ResearchValue.Known eventId ->
                    ...
                    | Some event when event.Body.ObservationId = body.ObservationId && event.EventHash = source.ArtifactSha256 -> ()
                    | Some _ -> fail "RESEARCH_SOURCE_INVALID" "harness-event hash or observation binding is invalid."
                    | None -> fail "RESEARCH_SOURCE_INVALID" "harness-event must reference a strictly earlier event in this observation."

    let private safeLedgerPath (root: string) (path: string) (allowMissing: bool) =
        let locations = Workspace.requireInitialized root
        let candidate = Workspace.requireSafePath locations "Research ledger" allowMissing path
        let relative = Workspace.relativePath locations candidate

        if not (relative.StartsWith(".ai/runtime/research/", StringComparison.Ordinal)) || not (relative.EndsWith(".jsonl", StringComparison.Ordinal)) then
    ...
    let ledgerPath (root: string) (observationId: string) =
        if not (observationIdPattern.IsMatch(observationId)) then fail "RESEARCH_SCHEMA_INVALID" "observationId has invalid syntax."
        Path.Combine(Path.GetFullPath(root), ".ai", "runtime", "research", "studies", ResearchContract.StudyId, "observations", observationId, "events.jsonl")

    let lockPath (ledger: string) = Path.Combine(Path.GetDirectoryName(Path.GetFullPath(ledger)), ".write.lock")
    ...
    let private verifyBytes (bytes: byte array) =
        ...
        while offset < bytes.Length && Option.isNone error do
            ...
            if lf < 0 then
                ...
                try
                    ...
                    if canonical.Length <> line.Length || not (Array.forall2 (=) canonical line) then fail "RESEARCH_CANONICAL_INVALID" "Record is not canonical JSON."
                    validateEventBody event.Body
                    validateEventBoundSources event.Body (List.ofSeq events)
                    if event.Sequence <> expectedSequence then fail "RESEARCH_CHAIN_INVALID" "sequence is not gapless."
                    if (event.PreviousEventHash |> ResearchValue.toOption |> Option.defaultValue ResearchContract.Unknown) <> previousHash then fail "RESEARCH_CHAIN_INVALID" "previousEventHash does not match."
                    ...
                    | Some known when known <> event.Body.ObservationId -> fail "RESEARCH_CHAIN_INVALID" "Observation IDs cannot mix in a ledger."
                    | _ -> ()

                    match evidenceClass with
                    | None -> evidenceClass <- Some event.Body.EvidenceClass
                    | Some known when known <> event.Body.EvidenceClass -> fail "EVIDENCE_CLASS_INVALID" "Evidence classes cannot mix in a ledger."
                    | _ -> ()
                    ...
                    | ResearchValue.Known clock, ResearchValue.Known value ->
                        ...
                        | true, last when value < last -> fail "RESEARCH_CLOCK_INVALID" "Monotonic time moved backwards within one clock."
                    ...
                    | "research.intervention.started" ->
                        ...
                        if not (interventionIds.Add(identifier)) then fail "INTERVENTION_INVALID" "interventionId is not unique."
                        match event.Body.MonotonicClockId, event.Body.MonotonicTimeNs with
                        | ResearchValue.Known clock, ResearchValue.Known value -> openInterventions.Add(identifier, (clock, value))
                        | _ -> fail "INTERVENTION_INVALID" "Intervention start requires a known monotonic clock and time."
                    | "research.intervention.ended" ->
                        let identifier = interventionId ()
                        match openInterventions.TryGetValue(identifier), event.Body.MonotonicClockId, event.Body.MonotonicTimeNs with
                        | (true, (startClock, startTime)), ResearchValue.Known endClock, ResearchValue.Known endTime when startClock = endClock && endTime >= startTime ->
                            let duration = requireProperty "durationMs" event.Body.Payload
                            let mutable durationMs = 0L
                            if duration.ValueKind <> JsonValueKind.Number || not (duration.TryGetInt64(&durationMs)) || durationMs <> (endTime - startTime) / 1_000_000L then
                                fail "INTERVENTION_INVALID" "durationMs must equal the same-clock monotonic start/end interval."
                            openInterventions.Remove(identifier) |> ignore
                        | _ -> fail "INTERVENTION_INVALID" "Intervention end requires exactly one open same-clock start."
        ...
        | Some(isTail, message, badOffset) when isTail ->
            ...
            ResearchLedgerStatus.TornTail, [ "TORN_TAIL"; message ], List.ofSeq events, int64 verifiedLength, Some(shaOrEmpty tail)
        | Some(_, message, _) -> ResearchLedgerStatus.Invalid, [ message ], List.ofSeq events, int64 verifiedLength, None
    ...
    let verify (root: string) (path: string) =
        ...
        let prefix = if verifiedLength = 0L then Array.empty else bytes[0 .. int verifiedLength - 1]
    ...
    let readVerified (root: string) (path: string) =
        ...
        if result.Status <> ResearchLedgerStatus.Valid then fail "RESEARCH_LEDGER_INVALID" (String.concat "; " result.Errors)
        result.Events
    ...
    let private writeNewFile (path: string) (bytes: byte array) =
        ...
        let temporary = Path.Combine(parent, $".{Path.GetFileName(path)}.{Guid.NewGuid():N}.tmp")

        try
            use stream = new FileStream(temporary, FileMode.CreateNew, FileAccess.Write, FileShare.None, max 1 bytes.Length, FileOptions.WriteThrough)
    ...
    let private redactBody (policy: RedactionPolicy) (draft: ResearchEventDraft) =
        ...
        let payloadBytes, payloadChanged = ResearchCanonical.redactAndCanonicalizePayload policy draft.Payload
        ...
        { body with RedactionStatus = if changed then ResearchValue.Known "applied" else body.RedactionStatus }
    ...
    let append (root: string) (path: string) (draft: ResearchEventDraft) =
        ...
        if int64 persistedPayloadBytes.Length > config.MaxEventPayloadBytes then fail "PAYLOAD_TOO_LARGE" "Redacted research payload exceeds maxEventPayloadBytes."
        validateEventBody body
        Directory.CreateDirectory(Path.GetDirectoryName(safePath)) |> ignore
        use _lock = lockLedger safePath

        if not (File.Exists(safePath)) then writeNewFile safePath Array.empty

        let current = verify root safePath

        match current.Status with
        | ResearchLedgerStatus.TornTail -> fail "TORN_TAIL" "Ledger has an incomplete or hash-invalid final record; explicit recovery is required."
        ...
        | ResearchValue.Known clock, ResearchValue.Known value ->
            let lastForClock =
                ...
                |> List.tryPick (fun previous ->
                    ...
                    | ResearchValue.Known previousClock, ResearchValue.Known previousValue when previousClock = clock -> Some previousValue
                    | _ -> None)

            match lastForClock with
            | Some previous when value < previous -> fail "RESEARCH_CLOCK_INVALID" "Monotonic time moved backwards within one clock."
            | _ -> ()
        | _ -> ()
        ...
        let previousHash = current.Events |> List.tryLast |> Option.map (fun event -> ResearchValue.Known event.EventHash) |> Option.defaultValue ResearchValue.Unknown
        let unhashed = { Body = body; Sequence = sequence; PreviousEventHash = previousHash; EventHash = String.replicate 64 "0" }
        let event = { unhashed with EventHash = computeHash unhashed }
        ...
        let candidateStatus, candidateErrors, _, _, _ = verifyBytes (Array.concat [ currentBytes; line ])

        if candidateStatus <> ResearchLedgerStatus.Valid then
            fail "RESEARCH_APPEND_INVALID" (String.concat "; " candidateErrors)

        let staged = Path.Combine(Path.GetDirectoryName(safePath), $".append.{Guid.NewGuid():N}.tmp")

        try
            use stage = new FileStream(staged, FileMode.CreateNew, FileAccess.Write, FileShare.None, line.Length, FileOptions.WriteThrough)
            stage.Write(line, 0, line.Length)
            stage.Flush(true)
            stage.Close()
            use ledger = new FileStream(safePath, FileMode.Append, FileAccess.Write, FileShare.Read, line.Length, FileOptions.WriteThrough)
    ...
    let recoverTo (root: string) (originalPath: string) (recoveredPath: string) (recoveryDraft: ResearchEventDraft) =
        ...
        if String.Equals(original, destination, StringComparison.Ordinal) then fail "RECOVERY_PATH_INVALID" "Recovery destination must be a new file."
        if File.Exists(destination) then fail "RECOVERY_PATH_INVALID" "Recovery destination already exists."
        ...
        if status <> ResearchLedgerStatus.TornTail then fail "RECOVERY_NOT_APPLICABLE" "Only a TORN_TAIL ledger can be recovered."
        if recoveryDraft.EventType <> "ledger.recovery.recorded" then fail "RECOVERY_EVENT_INVALID" "Recovery requires ledger.recovery.recorded."
        let originalHash = Internal.sha256Hex originalBytes
        let prefix = if prefixLength = 0L then Array.empty else originalBytes[0 .. int prefixLength - 1]
        let prefixHash = Internal.sha256Hex prefix
        let required name expected = if requireString name recoveryDraft.Payload <> expected then fail "RECOVERY_EVENT_INVALID" $"payload.{name} does not bind the damaged ledger."
        ...
        let persistedPayloadBytes = ResearchCanonical.canonicalizeElement body.Payload
        if int64 persistedPayloadBytes.Length > config.MaxEventPayloadBytes then fail "PAYLOAD_TOO_LARGE" "Redacted recovery payload exceeds maxEventPayloadBytes."
        validateEventBody body
        match List.tryLast prefixEvents with
        | Some previous when previous.Body.ObservationId <> body.ObservationId || previous.Body.EvidenceClass <> body.EvidenceClass -> fail "RECOVERY_EVENT_INVALID" "Recovery event does not match the verified prefix."
        | _ -> ()
        let previousHash = prefixEvents |> List.tryLast |> Option.map (fun event -> ResearchValue.Known event.EventHash) |> Option.defaultValue ResearchValue.Unknown
        let unhashed = { Body = body; Sequence = int64 prefixEvents.Length + 1L; PreviousEventHash = previousHash; EventHash = String.replicate 64 "0" }
        let event = { unhashed with EventHash = computeHash unhashed }
        let recoveredBytes = Array.concat [ prefix; canonicalEventBytes event |> ResearchCanonical.appendLf ]
        let candidateStatus, candidateErrors, _, _, _ = verifyBytes recoveredBytes
        if candidateStatus <> ResearchLedgerStatus.Valid then fail "RECOVERY_FAILED" (String.concat "; " candidateErrors)
        writeNewFile destination recoveredBytes
        let result = verify root destination
        if result.Status <> ResearchLedgerStatus.Valid then fail "RECOVERY_FAILED" (String.concat "; " result.Errors)
        result
