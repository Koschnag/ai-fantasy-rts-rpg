...
module ResearchCollector =
    let private writeImmutableDurably (path: string) (conflictCode: string) (bytes: byte array) =
        ...
        let temporary = Path.Combine(parent, "." + Path.GetFileName(path) + "." + Guid.NewGuid().ToString("N") + ".tmp")
        ...
        try
            ...
            with
            | :? IOException when File.Exists(path) ->
    ...
    let private safeCode (message: string) =
        ...
        let secretLike =
            [ "authorization"; "cookie"; "password"; "passwd"; "secret"; "token"; "api_key"; "private_key" ]
        ...
        if
            ...
            && not secretLike
            && candidate.Length <= 64
            && candidate |> Seq.forall (fun character -> Char.IsAsciiLetterOrDigit(character) || character = '_' || character = '-')
        then
            candidate
        else
            "COLLECTOR_FAILED"
    ...
    let healthIssues (root: string) =
        ...
        if Directory.Exists(directory)
           && (Directory.EnumerateFiles(directory, "HEALTH-*.json", SearchOption.TopDirectoryOnly) |> Seq.isEmpty |> not) then
            [ "COLLECTOR_HEALTH_FAILURES_PRESENT" ]
        else
            []

    let private withActive (root: string) (eventType: string) (action: ResearchActivationMarker -> ResearchStudyManifest -> ResearchAppendReceipt) =
        try
        ...
        | error ->
            ResearchCollectionResult.GapRecorded(recordGap root ResearchContract.Unknown eventType (error.GetType().Name))

    let private append (root: string) (marker: ResearchActivationMarker) (manifest: ResearchStudyManifest) (eventType: string) (sourceRefs: ResearchSourceReference list) (payload: JsonElement) (customize: ResearchEventDraft -> ResearchEventDraft) =
        let identity = ResearchGitImport.currentIdentity root
        let draft = ResearchRuntime.createDraft manifest identity eventType sourceRefs payload |> customize
    ...
    let recordStructured root eventType sourceRefs payload customize =
        withActive root eventType (fun marker manifest -> append root marker manifest eventType sourceRefs payload customize)
    ...
    let private persistJsonSource
        ...
        let path = Workspace.requireSafePath locations "Research immutable source receipt" true (Path.Combine(root, relative))
    ...
    let private freezeInterventionSource root marker sourceRef =
        let locations = Workspace.requireInitialized root
        let sourcePath = Workspace.requireSafePath locations "Research intervention source" false (Path.Combine(root, sourceRef))
        let config = HarnessConfig.load locations
        let bytes = Internal.safeReadAllText sourcePath config.MaxEventPayloadBytes |> Constants.Utf8NoBom.GetBytes
    ...
    let recordRunStarted root runId actorId =
        withActive root "agent.run.started" (fun marker manifest ->
            let started = RunStore.eventsStrict root runId |> List.head |> fun event -> RunStore.eventByReceipt root runId event.Sequence "run.started" event.EventHash
            ...
            let source = ResearchRuntime.harnessRunEventSource root runId started.Sequence started.EventType started.EventHash
            ...
            append root marker manifest "agent.run.started" [ source ] payload (fun draft ->
                { draft with
                    ...
                    ModelId = if observedModel = ResearchContract.Unknown then ResearchValue.Unknown else ResearchValue.Known observedModel }))
    ...
    let recordRunFinished (root: string) (runId: string) (sequence: int64) (eventHash: string) =
        withActive root "agent.run.finished" (fun marker manifest ->
            ...
            let source = ResearchRuntime.harnessRunEventSource root runId finished.Sequence finished.EventType finished.EventHash
            ...
            append root marker manifest "agent.run.finished" [ source ] payload (fun draft ->
                { draft with RunId = ResearchValue.Known runId; Result = result }))

    let private passthroughEventTypes =
        Set.difference
            ResearchContract.EventTypes
            (set
                [ "protocol.frozen"; "observation.started"; "agent.run.started"; "agent.run.finished"
                  "gate.started"; "gate.finished"; "ledger.recovery.recorded"; "tool.finished"
                  "research.intervention.started"; "research.intervention.ended"; "research.intervention.recorded"
                  "outcome.observed"; "observation.closed" ])

    let recordHarnessEvent root runId sequence sourceEventHash eventType =
        withActive root eventType (fun marker manifest ->
            let authoritative = ResearchRuntime.authoritativeEvent root runId sequence eventType sourceEventHash
            ...
            if not (Set.contains authoritative.EventType passthroughEventTypes) then
                ...
                append root marker manifest authoritative.EventType [ source ] (canonicalDocument.RootElement.Clone()) (fun draft ->
                    { draft with RunId = ResearchValue.Known runId }))

    let recordHarnessEvidence root runId sequence sourceEventHash =
        withActive root "tool.finished" (fun marker manifest ->
            let authoritative = ResearchRuntime.authoritativeEvent root runId sequence "evidence.recorded" sourceEventHash
            let canonical = ResearchCanonical.canonicalizeElement (JsonDocument.Parse(authoritative.Payload).RootElement)
            ...
            append root marker manifest "tool.finished" [ source ] researchPayload (fun draft ->
                { draft with RunId = ResearchValue.Known runId; Result = result; ExitCode = exitCode }))
    ...
    let private validateGateId (gateId: string) =
        if
            ...
            || gateId
               |> Seq.exists (fun character ->
                   not (Char.IsAsciiLetterOrDigit(character)) && character <> '-' && character <> '_')
        then
    ...
    let private unmatchedGateStart root observationId gateId =
        let events = ResearchLedger.readVerified root (ResearchLedger.ledgerPath root observationId)
        let finishes =
            events
            |> List.filter (fun event -> event.Body.EventType = "gate.finished" && payloadString "gateId" event = gateId)
        ...
        |> List.tryLast
        |> Option.defaultWith (fun () -> Internal.fail "RESEARCH_GATE_INVALID: no unmatched authoritative gate start exists.")
    ...
    let recordVerificationStarted (root: string) (gateId: string) =
        ...
        withActive root "gate.started" (fun marker manifest ->
            ...
            let identity = ResearchGitImport.currentIdentity root
            let targetTree = if identity.WorktreeClean then identity.HeadTreeId else ResearchContract.Unknown
            let prior = ResearchLedger.readVerified root (ResearchLedger.ledgerPath root marker.ObservationId)
            let attempt =
                prior
                |> List.filter (fun event -> event.Body.EventType = "gate.started" && payloadString "gateId" event = gateId)
                |> List.length
            ...
            append root marker manifest "gate.started" [ source ] payload (fun draft ->
                { draft with RetryIndex = ResearchValue.Known(attempt - 1L) }))
        ...
        withActive root "gate.finished" (fun marker manifest ->
            ...
            let payload =
                ResearchRuntime.payload (fun (writer: Utf8JsonWriter) ->
                    ...
                    writer.WriteString("gateId", gateId)
                    writer.WriteString("targetTreeId", targetTree))
            let result = if valid then ResearchValue.Known "pass" else ResearchValue.Known "fail"
            let finished =
                append root marker manifest "gate.finished" [ source ] payload (fun draft ->
                    { draft with Result = result; RetryIndex = ResearchValue.Known(attempt - 1L) })
    ...
    let interventionStart (root: string) (observationId: string) (category: string) (sourceRef: string) (reasonCode: string) =
        let marker = ResearchActivation.tryActive root |> Option.defaultWith (fun () -> Internal.fail "ACTIVE_OBSERVATION_MISSING: no active observation.")
    ...
    let interventionEnd root observationId interventionId sourceRef =
        ...
        if events |> List.exists (fun event -> event.Body.EventType = "research.intervention.ended" && event.Body.Payload.GetProperty("interventionId").GetString() = interventionId) then
        ...
        let duration =
            ...
            | ResearchValue.Known first, ResearchValue.Known firstClock, ResearchValue.Known last, ResearchValue.Known lastClock
                when firstClock = lastClock && last >= first -> ResearchValue.Known((last - first) / 1_000_000L)
            | _ -> ResearchValue.Unknown
    ...
    let interventionRecord (root: string) (observationId: string) (category: string) (sourceRef: string) (reasonCode: string) =
        let marker = ResearchActivation.tryActive root |> Option.defaultWith (fun () -> Internal.fail "ACTIVE_OBSERVATION_MISSING: no active observation.")
